...
import com.yt.project.common.COSClientUtil;
...
import com.yt.project.mapper.UserMapper;
...
import static com.yt.project.constant.UserConstant.ADMIN_ROLE;
import static com.yt.project.constant.UserConstant.USER_LOGIN_STATE;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
     ...
     * 盐值，混淆密码
     */
    private static final String SALT = "yt";
    ...
    public User getInvokeUser(String accessKey, String secretKey) {
        ...
        QueryWrapper<User> queryWrapper = new QueryWrapper<>();
        queryWrapper.and(userQueryWrapper -> userQueryWrapper.eq("accessKey", accessKey).or().eq("secretKey", secretKey));
        return this.getOne(queryWrapper);
    }

    @Override
    public AkSkVO getUserAccessKeySecretKey(HttpServletRequest request) {
        ...
        akSkVO.setAccessKey(userInfo.getAccessKey());
        akSkVO.setSecretKey(userInfo.getSecretKey());
        return akSkVO;
    }

    @Override
    public Boolean updateAkSk(HttpServletRequest request) {
        // 获取用户信息
    ...
    public String uploadUserAvatar(MultipartFile file, HttpServletRequest request) {
        ...
        String upload = null;
        try {
            upload = COSClientUtil.upload(file);
            log.info("上传头像成功:" + upload);
            User user = getLoginUser(request);
            UpdateWrapper<User> updateWrapper = new UpdateWrapper<>();
            updateWrapper.set("userAvatar", upload);
